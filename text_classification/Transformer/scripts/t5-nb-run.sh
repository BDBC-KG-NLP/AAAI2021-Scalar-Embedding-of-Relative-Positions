  ...
  do
  {
   ...
   do
   {
    ...
    python train_cls.py \
                  ...
                  --t5_bucket=${t5_bucket} \
                  --trail=${trail} \
                  --is_training=False #> ${log_dir}/${model_name}_reber_L-1_H-${num_attention_heads}-${training_nums}-${lr}-${t5_bucket}-${trail} &
   }
   done
  }
  done
...
do
{
  ...
  do
  {
    srun --gres=gpu:1 python train_cls.py \
                            --model_name=${model_name} \
                            --data=process_cls_50 \
                            --num_epochs=100 \
                            --learning_rate=1e-4 \
