...
class T5_Model(object):
  ...
  def forward(self):
    ...
    with tf.variable_scope("Embedding_Encoder_Layer"):
      ...
      c_context_position = tf.range(self.c_maxlen,dtype=tf.int32)[:,None]
      c_memory_postion = tf.range(self.c_maxlen, dtype=tf.int32)[None,:]
      c_relative_position = c_memory_postion - c_context_position
      
      c_rp_bucket = relative_position_bucket(c_relative_position,
                                             num_buckets=config.t5_num_buckets,
                                             max_distance=config.t5_max_distance)
      
      self.c_t5_bias_mat = tf.get_variable('c_t5_bias_mat', 
                                                   [config.t5_num_buckets,nh],
                                                   initializer=tf.random_uniform_initializer())
      ## [batch, num_heads, query_length, memory_length]
      c_t5_bias = compute_bias(c_rp_bucket, self.c_t5_bias_mat) #(qlen, klen, 1, num_heads)
      print('c_t5_bias:',c_t5_bias)
      
      head_mask = np.zeros((nh,config.para_limit,config.para_limit))
      #hidx=[0,5,3,4,1,2,6,7]
      low2high=[7,6,2,1,4,3,5,0]
      for tt in range(PRTIN_ATT):
        head_mask[low2high[tt],:,:]=np.ones((config.para_limit,config.para_limit))
      
      c_t5_bias=c_t5_bias* tf.slice(tf.constant(head_mask,tf.float32),
                                    [0,0,0],
                                    [8,self.c_maxlen,self.c_maxlen])
      ...
      q_context_position = tf.range(self.q_maxlen,dtype=tf.int32)[:,None]
      q_memory_postion = tf.range(self.q_maxlen, dtype=tf.int32)[None,:]
      q_relative_position = q_memory_postion - q_context_position
      q_rp_bucket = relative_position_bucket(q_relative_position,
                                             num_buckets=config.t5_num_buckets,
                                             max_distance=config.t5_max_distance)
      ...
      self.q_t5_bias_mat = tf.get_variable('q_t5_bias_mat', 
                                                   ...
                                                   initializer=tf.random_uniform_initializer())
      ## [batch, num_heads, query_length, memory_length]
      q_t5_bias = compute_bias(q_rp_bucket, self.q_t5_bias_mat) #(qlen, klen, 1, num_heads)
      
      head_mask = np.zeros((nh,config.para_limit,config.para_limit))
      #hidx=[5,3,7,6,1,2,0,4]
      low2high=[4,0,2,1,6,7,3,5]
      for tt in range(PRTIN_ATT):
        head_mask[low2high[tt],:,:]=np.ones((config.para_limit,config.para_limit))
      
      q_t5_bias=q_t5_bias* tf.slice(tf.constant(head_mask,tf.float32),
                                    [0,0,0],
                                    [8,self.q_maxlen,self.q_maxlen])
    ...
    '''
    hidx_list = [[2,5,4,0,3,7,1,6],
                 [5,4,3,7,0,2,6,1],
                 [5,7,3,4,6,2,0,1]]'''
    low2high_list=[[6,1,7,3,0,4,5,2],
                   [1,6,2,0,7,3,4,5],
                   [1,0,2,6,4,3,7,5]]
    ...
    with tf.variable_scope("Model_Encoder_Layer"):
      ...
      for i in range(3):
        ...
        head_mask = np.zeros((nh,config.para_limit,config.para_limit))
        for tt in range(PRTIN_ATT):
          head_mask[low2high_list[i][tt],:,:]=np.ones((config.para_limit,config.para_limit))
        
        head_mask=tf.slice(tf.constant(head_mask,tf.float32),
                                    [0,0,0],
                                    [8,self.c_maxlen,self.c_maxlen])
        ...
        layer_weights,model_c =residual_block(self.enc[i],
                      ...
                      dropout = self.dropout,
                      t5_bias=head_mask*compute_bias(c_rp_bucket, self.model_c_t5_bias_mat[i]))
