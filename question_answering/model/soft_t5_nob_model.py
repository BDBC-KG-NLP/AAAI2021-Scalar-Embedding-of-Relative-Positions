...
def polynomial_score(f,soft_t5_width=40,
...
def compute_bias(num_heads, qlen, klen,
  ...
  for headi in range(num_heads):
    soft_t5_bias_list.append(polynomial_score(rp_bucket[:,:,headi],
                                              soft_t5_width,name=name+'-head-'+str(headi))
                             )
...
class Soft_T5_NoB_Model(object):
  ...
  def forward(self):
    ...
    with tf.variable_scope("Embedding_Encoder_Layer"):
      c_soft_t5_rd_bucket_embedding =tf.get_variable('c_soft_t5_rd_bucket_embedding', 
                                    [2*config.para_limit,
                                     nh],
                                     initializer=tf.random_uniform_initializer(),
                                     trainable=True
                                    )
      self.c_soft_t5_rd_bucket_mat=tf.sigmoid(c_soft_t5_rd_bucket_embedding)
        
      self.c_t5_bias = compute_bias(nh,
                         config.para_limit,config.para_limit,
                         self.c_soft_t5_rd_bucket_mat,
                         soft_t5_width=40,
      ...
      q_soft_t5_rd_bucket_embedding =tf.get_variable('q_soft_t5_rd_bucket_embedding', 
                                    [2*config.ques_limit,
                                     nh],
                                     initializer=tf.random_uniform_initializer(),
                                     trainable=True
                                    )
      self.q_soft_t5_rd_bucket_mat=tf.sigmoid(q_soft_t5_rd_bucket_embedding)
        
      self.q_t5_bias = compute_bias(nh,
                                    config.ques_limit,config.ques_limit,
                                    self.q_soft_t5_rd_bucket_mat,
                                    soft_t5_width=40,
                                    dropout_prob=self.dropout,
                                    activation=config.soft_t5_activation,
                                    bidirectional=True,
    ...
    self.model_soft_t5_rd_bucket_list=[]
    with tf.variable_scope("Model_Encoder_Layer"):
      ...
      for i in range(3):
        ...
        cq_soft_t5_rd_bucket_embedding =tf.get_variable('cq_soft_t5_rd_bucket_embedding'+str(i), 
                                    [2*config.para_limit,
                                     nh],
                                     initializer=tf.random_uniform_initializer(),
                                     trainable=True
                                    )
        cq_soft_t5_rd_bucket_mat=tf.sigmoid(cq_soft_t5_rd_bucket_embedding)
        
        self.model_soft_t5_rd_bucket_list.append(cq_soft_t5_rd_bucket_mat)
        c_t5_bias = compute_bias(nh,
                                 config.para_limit,config.para_limit,
                                 cq_soft_t5_rd_bucket_mat,
                                 soft_t5_width=40,
                                 dropout_prob=self.dropout,
                                 activation=config.soft_t5_activation,
                                 bidirectional=True,
