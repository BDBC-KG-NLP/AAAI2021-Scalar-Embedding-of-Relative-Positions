...
def residual_block(inputs, num_blocks, num_conv_layers, kernel_size, mask = None,
                   ...
                   reuse = None, bias = True, dropout = 0.0,t5_bias=None):
    layer_weights=[]
    with tf.variable_scope(scope, reuse = reuse):
        ...
        for i in range(num_blocks):
            #we also add the time signal to capture the global dependency...
            outputs = add_timing_signal_1d(outputs)  #t5 positional embeddings
            #we only rely on the position embedding to capture dependency infos...
            '''
            ...
            '''
            weights,outputs, sublayer = self_attention_block(outputs, num_filters,
                                  seq_len,mask = mask, num_heads = num_heads,
...
def self_attention_block(inputs, num_filters, seq_len, mask = None, 
                         ...
                         sublayers = (1, 1),t5_bias=None):
    with tf.variable_scope(scope, reuse = reuse):
        l, L = sublayers
        # Self attention
        outputs = norm_fn(inputs, scope = "layer_norm_1", reuse = reuse)
        outputs = tf.nn.dropout(outputs, 1.0 - dropout)
        weights,outputs = multihead_attention(outputs, num_filters,
...
def multihead_attention(queries, units, num_heads,
    ...
    with tf.variable_scope(scope, reuse = reuse):
        ...
        weights,x = dot_product_attention(Q,K,V,
                                  bias = bias,
                                  seq_len = seq_len,
                                  mask = mask,
        ...
        return weights,combine_last_two_dimensions(tf.transpose(x,[0,2,1,3]))
    ...
    with tf.variable_scope(scope, default_name="dot_product_attention", reuse = reuse):
        ...
        if t5_bias!=None:
            if type(t5_bias)!=list:
                ...
                queries = tf.transpose(q,[1,2,0,3])  #(N,F,B,H)
        ...
        if type(t5_bias)!=list or t5_bias==None:
            #(B,N,F,dv)
            return weights,tf.matmul(weights, v)
        else:
            #(N,F,B,T)
            re_weights=tf.transpose(weights,[1,2,0,3])
            print(re_weights)
            print(rpr_v)
            ...
            outputs_part2 = tf.transpose(outputs_part2,[2,0,1,3])
            
            print('outputs_part2:',outputs_part2)
            return weights,tf.matmul(weights, v) + outputs_part2
