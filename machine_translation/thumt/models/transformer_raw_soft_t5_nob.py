...
def transformer_encoder(inputs, bias, params,dtype=None, scope=None):
    ...
    with tf.variable_scope(scope, default_name="encoder", dtype=dtype,
        ...
        encoder_soft_t5_rd_bucket_embedding =tf.get_variable('encoder_soft_t5_rd_bucket_embedding', 
                                    [2*params.max_length,
                                     params.num_heads],
        ...
        encoder_soft_t5_rd_bucket_mat=tf.sigmoid(encoder_soft_t5_rd_bucket_embedding)
        
        soft_t5_bias = layers.soft_t5_attention_nob.compute_bias(params.num_heads,
                         params.max_length,params.max_length,
                         encoder_soft_t5_rd_bucket_mat,
        ...
        for layer in range(params.num_encoder_layers):
            with tf.variable_scope("layer_%d" % layer):
                with tf.variable_scope("self_attention"):
                    y = layers.soft_t5_attention_nob.multihead_attention(
...
def transformer_decoder(inputs, memory, bias, mem_bias, params,state=None,
    ...
    with tf.variable_scope(scope, default_name="decoder", dtype=dtype,
        ...
        print('params.soft_t5_act:',params.soft_t5_act)
        decoder_soft_t5_rd_bucket_embedding =tf.get_variable('decoder_soft_t5_rd_bucket_embedding', 
                                    [2*params.max_length,
                                     params.num_heads],
                                     initializer=tf.random_uniform_initializer(),
                                     trainable=True
                                    )
        decoder_soft_t5_rd_bucket_mat=tf.sigmoid(decoder_soft_t5_rd_bucket_embedding)
        
        next_state = {}
        
        soft_t5_bias = layers.soft_t5_attention_nob.compute_bias(params.num_heads,
                         params.max_length,params.max_length,
                         decoder_soft_t5_rd_bucket_mat,
                         l1_width=params.l1_width,
                         l2_width=params.l2_width,
                         stddev=params.stddev,
                         dropout_prob=params.soft_t5_attention_dropout,
