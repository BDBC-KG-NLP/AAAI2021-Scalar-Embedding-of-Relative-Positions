...
Created on Mon Apr 27 21:23:56 2020
...
def transformer_encoder(inputs, bias, params,dtype=None, scope=None):
    ...
    with tf.variable_scope(scope, default_name="encoder", dtype=dtype,
        ...
        klen = tf.shape(inputs)[1]
        
        print('params.soft_t5_act:',params.soft_t5_act)
        
        encoder_alpha, encoder_beta = layers.soft_t5_attention.get_clip(params.num_heads,
                                    A=tf.cast(1/params.max_length,dtype=tf.float32),
                                    config=params,
                                    name='soft_t5_encoder',
                                  )
        encoder_alpha=tf.identity(encoder_alpha,'encoder_alpha')
        encoder_beta=tf.identity(encoder_beta,'encoder_beta')
        
        soft_t5_bias = layers.soft_t5_attention.compute_bias(params.num_heads,
                         params.max_length,params.max_length,
                         encoder_alpha,
                         encoder_beta,
                         l1_width=params.l1_width,
                         l2_width=params.l2_width,
                         stddev=params.stddev,
                         dropout_prob=params.soft_t5_attention_dropout,
                         activation=params.soft_t5_act,
                         bidirectional=True,
                        )
        encoder_soft_t5_bias=tf.identity(soft_t5_bias,'soft_t5_bias')
        head_mask = np.zeros((params.num_heads,params.max_length,params.max_length))
        #high2low=[1,6,0,5,2,7,4,3]
        low2high=[3,4,7,2,5,0,6,1]
        for tt in range(8):
          print('tt:',tt)
          head_mask[low2high[tt],:,:]=np.ones((params.max_length,params.max_length))
        
        tf.logging.info(soft_t5_bias.name)
        encoder_soft_t5_bias=encoder_soft_t5_bias*head_mask
        
        ##bool(layer==0) or params.every_layer_has_rp_bias,
        #different layers by using different embeddings?
        for layer in range(params.num_encoder_layers):
            with tf.variable_scope("layer_%d" % layer):
                with tf.variable_scope("self_attention"):
                    y = layers.soft_t5_attention.multihead_attention(
                        ...
                        has_relative_attention_bias=bool(layer==0) or params.every_layer_has_rp_bias,
                        soft_t5_bias=encoder_soft_t5_bias[:,:,:qlen,:klen]
                    )
                    y = y["outputs"]
...
def transformer_decoder(inputs, memory, bias, mem_bias, params,state=None,
    ...
    with tf.variable_scope(scope, default_name="decoder", dtype=dtype,
        ...
        decoder_self_alpha, decoder_self_beta = layers.soft_t5_attention.get_clip(params.num_heads,
                                 A=tf.cast(1/params.max_length,dtype=tf.float32),
                                 config=params,
                                name='soft_t5_decoder')
        
        decoder_self_alpha=tf.identity(decoder_self_alpha,'decoder_alpha')
        decoder_self_beta=tf.identity(decoder_self_beta,'decoder_beta')
        ...
        decoder_soft_t5_bias=tf.identity(soft_t5_bias,'soft_t5_bias')
            ...
            with tf.variable_scope(layer_name):
                ...
                with tf.variable_scope("self_attention"):
                    y = layers.soft_t5_attention.multihead_attention(
                        ...
                        state=layer_state,
                        has_relative_attention_bias=bool(layer==0) or params.every_layer_has_rp_bias,
                        soft_t5_bias=decoder_soft_t5_bias[:,:,:qlen,:klen]
                    )
                    
                    if layer_state is not None:
                        next_state[layer_name] = y["state"]
                    if layer==0:
                        ret_soft_t5_bias_self=y['soft_t5_bias']
...
def encoding_graph(features, mode, params):
    if mode != "train":
        ...
        params.soft_t5_attention_dropout=0.0
...
def decoding_graph(features, state, mode, params):
    ...
    if mode != "infer":
        ...
        decoder_output, decoder_state = transformer_decoder(
            decoder_input,encoder_output,dec_attn_bias, enc_attn_bias,
            params,
            state=state["decoder"])
...
class Transformer(NMTModel):
    ...
    @staticmethod
    def get_parameters():
        #this is for wmt2014 en-de tasks
