...
def transformer_encoder(inputs, bias, params, dtype=None, scope=None):
    with tf.variable_scope(scope, default_name="encoder", dtype=dtype,
                           values=[inputs, bias]):
        #x: [batch, length_q, depth_q]
        x = inputs
        
        #we utilize the shared t5 parameters...
        en_self_relative_attention_bias = tf.get_variable('en_t5_bias_mat',
                        [params.num_buckets,params.num_heads],
                        initializer=tf.random_uniform_initializer()
                      )
        
        relative_position=layers.t5_attention.get_t5_relative_position(params.max_length,
                                                                params.max_length)
        rp_bucket=layers.t5_attention.relative_position_bucket(relative_position,
                                               num_buckets=params.num_buckets,
                                               max_distance=params.max_distance,
                                               bidirectional=True)
        t5_bias = layers.t5_attention.compute_bias(rp_bucket, en_self_relative_attention_bias)
        encoder_t5_bias=tf.identity(t5_bias,'encoder_t5_bias')
        
        head_mask = np.zeros((params.num_heads,params.max_length,params.max_length))
        #high2low=[6,4,7,1,5,3,2,0]
        low2high=[0,2,3,5,1,7,4,6]
        for tt in range(8):
          print('tt:',tt)
          head_mask[low2high[tt],:,:]=np.ones((params.max_length,params.max_length))
      
        encoder_t5_bias = encoder_t5_bias*head_mask
        ...
        for layer in range(params.num_encoder_layers):
            with tf.variable_scope("layer_%d" % layer):
                ...
                with tf.variable_scope("self_attention"):
                    y = layers.t5_attention.multihead_attention(
                        ...
                        t5_bias=encoder_t5_bias[:,:,:qlen,:klen]
                    )
                    y = y["outputs"]
...
def transformer_decoder(inputs, memory, bias, mem_bias, params, state=None,
    ...
    with tf.variable_scope(scope, default_name="decoder", dtype=dtype,
        ...
        #scalar information
        de_self_relative_attention_bias = tf.get_variable(
                            'de_self_relative_attention_bias', 
                            [params.num_buckets,params.num_heads],
                            initializer=tf.random_uniform_initializer(),
                            )
        ...
        rp_bucket=layers.t5_attention.relative_position_bucket(relative_position,
                                               ...
                                               max_distance=params.max_distance,
                                               bidirectional=False)
        t5_bias = layers.t5_attention.compute_bias(rp_bucket, de_self_relative_attention_bias)
        
        decoder_t5_bias=tf.identity(t5_bias,'decoder_t5_bias')
        qlen = tf.shape(inputs)[1]
        klen = tf.shape(inputs)[1]
            ...
            with tf.variable_scope(layer_name):
                ...
                with tf.variable_scope("self_attention"):
                    y = layers.t5_attention.multihead_attention(
                        ...
                        state=layer_state,
                        has_relative_attention_bias=bool(layer==0) or params.every_layer_has_rp_bias,
                        t5_bias=decoder_t5_bias[:,:,:qlen,:klen]
                    )
...
class Transformer(NMTModel):
    ...
    def get_parameters():
        ...
        params = tf.contrib.training.HParams(
            ...
            #weather to share the embedding and softmax weights
